...
The `localize` template tag allows performing localization of values such as dates, numbers, and times by using the [I18n gem](https://i18n.github.io/localization.html), which is leveraged by Marten for its [internationalization features](../../i18n/introduction.md). It must take at least one argument (the value to localize) followed by an optional `format` keyword argument.
